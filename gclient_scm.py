...
class GitWrapper(SCMWrapper):
    ...
    def _FetchAndReset(self, revision, file_list, options):
        ...
        # TODO(tim): why can't we use the local branch name?
    ...
    """
        ...
        if target_rev.startswith(('refs/heads/', 'refs/branch-heads')):
            ...
            # remote ref.
            remote_ref = ''.join(scm.GIT.RefToRemoteRef(target_rev,
                                                        self.remote))
...
class P4Wrapper(SCMWrapper):
  ...
  """
    ...
    def __init__(self,
                 ...
                 out_fh=None,
                 out_cb=None):
        super(P4Wrapper, self).__init__(url=url,
                                        root_dir=root_dir,
                                        relpath=relpath,
                                        out_fh=out_fh,
                                        out_cb=out_cb)
        self.context = scm_p4.P4Context.for_checkout(self.checkout_path,
                                                     self.url)
    ...
    def revinfo(self, options, args, file_list):
        output = self.context.run_marshalled(
    ...
    def status(self, options, args, file_list):
        if not os.path.isdir(self.checkout_path):
            self.Print('________ couldn\'t run status in %s:\n'
                       'The directory does not exist.' % self.checkout_path)
            return
        output = self.context.run_marshalled(['status', '-m'],
                                             check=False,
                                             cwd=self.checkout_path)
    ...
    def update(self, options, args, file_list):
        ...
        if options.force or options.reset:
            self.context.run_marshalled(['revert', '//...'],
                                        check=True,
                                        cwd=self.checkout_path)
        ...
        output = self.context.run_marshalled(args,
                                             check=True,
                                             cwd=self.checkout_path)
    ...
    def _CreateClient(self):
        ...
        scm_p4.create_client(self.context,
                             self.context.client,
                             self.checkout_path,
                             options=[
                                 'noallwrite', 'clobber', 'nocompress',
                                 'unlocked', 'modtime', 'rmdir'
                             ],
                             stream=stream)
