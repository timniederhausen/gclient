...
PerforceWorkspaceUrl = namedtuple('PerforceWorkspaceUrl', 'port username password path depot_type changelist')
...
def parse_workspace_url(url: str):
  ...
  return PerforceWorkspaceUrl(p4port, components.username, components.password,
                              '//' + components.path[1:], depot_type,
                              changelist)
...
def unmarshal_p4_output(stdout: bytes, check=False):
  ...
  try:
    while True:
      ...
      if check and result[b'code'] == b'error' and result[b'severity'] >= E_FAILED:
        raise Exception(f'Failed P4 call: {result.get(b"data", result)}')
...
def encode_marshalled_command_input(input: typing.Dict[typing.AnyStr, typing.AnyStr]):
...
def format_client_name(checkout_path: str, parsed_url: PerforceWorkspaceUrl):
  ...
  return client_name_template.format(user=parsed_url.username or os.environ.get('P4USER'),
...
class P4Context(P4Environment):
  ...
  def call(self, cmdargs: typing.List[str], **kwargs):
    return subprocess.run(['p4'] + cmdargs, text=True,
                          env=self.prepare_environment(kwargs), **kwargs)

  def run(self, cmdargs: typing.List[str], **kwargs):
    proc = subprocess.run(['p4'] + cmdargs, stdout=subprocess.PIPE, text=True,
                          env=self.prepare_environment(kwargs), **kwargs)
    return proc.stdout

  def run_marshalled(self, cmdargs: typing.List[str], input: dict = None, check=False, **kwargs):
    if input:
    ...
    logging.debug('P4: \'%s\' for \'%s\'', ' '.join(args), env.get('P4CLIENT', 'none'))

    # For -G: https://community.perforce.com/s/article/3518
    proc = subprocess.run(args,
                          input=dumped_input, stdout=subprocess.PIPE,
                          env=env, **kwargs)
...
def make_client_spec(ctx: P4Context, name: str, root: str, owner: str = None, host: str = None, description: str = None,
                     options: typing.List[str] = None,
                     submit_options: typing.List[str] = None,
                     eol='local', stream: str = None, views: typing.Dict[typing.AnyStr, typing.AnyStr] = None):
...
def create_client(ctx: P4Context, name: str, root: str, owner: str = None, host: str = None, description: str = None,
                  options: typing.List[str] = None,
                  submit_options: typing.List[str] = None,
                  eol='local', stream: str = None, views: typing.Dict[typing.AnyStr, typing.AnyStr] = None):
  spec = make_client_spec(ctx, name, root, owner, host, description, options, submit_options, eol, stream, views)
  return ctx.run_marshalled(['client', '-i'], input=spec, check=True)


def create_change(ctx: P4Context, description: str = '', type: str = None, user: str = None):
...
def delete_change(ctx: P4Context, cl: int) -> bool:
  ...
  if re.search(r'Change (.+) deleted.', output, re.M) and 'can\'t be deleted.' not in output:
    return True
  return False
...
def submit_change(ctx: P4Context, cl: int):
  for _ in range(40):
    ...
    try:
      output = ctx.run(['submit', '-c', str(cl), '-f', 'submitunchanged'], check=True)
      output = output.strip()
      if output.endswith('submitted.'):
        new_cl = 0
        if output.endswith('and submitted.'):
          mo = re.search(r'renamed change (.+) and submitted.$', output, re.M)
          new_cl = int(mo.group(1))
        else:
          mo = re.search(r'Change (.+) submitted.$', output, re.M)
          new_cl = int(mo.group(1))
        if new_cl < cl:
          raise Exception(f'Failed to get change number from submitted: {output}')
        return new_cl
